package br.unifor.poo.view.util;
...
public class Misc {
	...
	private static <T> Field getFieldPosition(T n, int column) { return Misc.getChildAndSuperFields(n)[column]; }
	public static <T> Object getFieldPositionObject(T n, int column) {
		Object o = null;
		try { o = Misc.getFieldPosition(n,column).get(n); } 
	...
	public static <T> T[] concatTwoArrays(T n, T[] first, T[] second) {
		...
		for (int i = 0; i < second.length; i++) result[i+first.length] = second[i]; 
		return result;
	}
	public static void setFont(Component ... list) { for (Component c : list) c.setFont(Misc.FONT); }
}
